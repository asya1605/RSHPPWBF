<aside class="bg-gradient-to-b from-[#002080] via-[#00185e] to-[#001040] text-white w-64 h-screen fixed top-0 left-0 flex flex-col z-40 shadow-2xl">
  ...
  <div class="p-6 border-b border-white/10 flex-shrink-0">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="font-black text-lg">Dokter Panel</h1>
  ...
  <div class="px-4 py-3 bg-white/5 border-b border-white/10 flex-shrink-0">
    <div class="flex items-center gap-3">
      ...
      <div class="flex-1 min-w-0">
        <p class="font-semibold text-sm truncate">{{ auth()->user()->nama ?? 'Dokter' }}</p>
        <p class="text-xs text-blue-300 truncate">Dokter</p>
      </div>
    </div>
  </div>
  ...
  <nav class="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar">
    ...
    <a href="{{ route('dashboard.dokter') }}" 
       class="sidebar-link {{ request()->routeIs('dashboard.dokter') ? 'active' : '' }}">
    ...
    <div class="pt-4 pb-2">
      <div class="flex items-center gap-2 px-3">
        ...
        <span class="text-[10px] uppercase tracking-widest text-blue-300 font-bold">Menu Dokter</span>
        <div class="h-px flex-1 bg-gradient-to-r from-transparent via-white/20 to-transparent"></div>
      </div>
    </div>
    ...
    <a href="{{ route('dokter.pasien.index') }}" 
       class="sidebar-link {{ request()->is('dokter/pasien*') ? 'active' : '' }}">
      <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <a href="{{ route('dokter.rekam-medis.index') }}" 
       class="sidebar-link {{ request()->is('dokter/rekam-medis*') ? 'active' : '' }}">
      <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <a href="{{ route('dokter.profil.index') }}" 
       class="sidebar-link {{ request()->is('dokter/profil*') ? 'active' : '' }}">
      <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
{{-- STYLES (pakai kalau belum di-include dari layout admin) --}}
<style>
  @keyframes float {
