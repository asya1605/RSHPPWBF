...
<section class="min-h-[90vh] bg-[#f5f7ff]">
  ...
  <div class="max-w-6xl mx-auto py-12 px-6">
    ...
    <div class="flex justify-end mb-5">
      <a href="{{ route('admin.pet.create') }}" class="bg-[#002080] hover:bg-[#00185e] text-white px-5 py-2 rounded-lg text-sm font-medium">
        ➕ Tambah Pet
      </a>
    </div>
        ...
        <tbody class="divide-y divide-gray-100">
          @forelse($pets as $p)
            <tr class="hover:bg-gray-50 transition">
              ...
              <td class="py-3 px-4">{{ $p->jenis_kelamin === 'M' ? 'Jantan' : 'Betina' }}</td>
              <td class="py-3 px-4">{{ $p->tanggal_lahir ?? '-' }}</td>
              <td class="py-3 px-4 text-center">
                ...
                <a href="{{ route('admin.pet.destroy', $p->idpet) }}"
                   onclick="return confirm('Yakin ingin menghapus pet ini?')"
                   class="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded text-xs">Hapus</a>
              </td>
            </tr>
          @empty
            <tr><td colspan="7" class="text-center py-5 text-gray-500">Belum ada data pet.</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
