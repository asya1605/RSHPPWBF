...
<section class="min-h-[90vh] bg-[#f5f7ff] flex justify-center items-center px-4 py-10">
  <div class="bg-white shadow-lg rounded-2xl w-full max-w-md border border-gray-200 px-7 py-6">
    ...
    <div class="flex items-start justify-between gap-3 mb-4 border-b border-gray-100 pb-3">
      <div>
        <h2 class="text-lg font-bold text-[#002080]">✏️ Edit Kategori Klinis</h2>
        <p class="text-xs text-gray-500 mt-1">
          Perbarui nama kategori klinis yang digunakan untuk pengelompokan diagnosa atau tindakan medis.
        </p>
      </div>
    ...
    {{-- Pesan Sukses (jika ada redirect balik ke sini) --}}
    @if (session('success'))
      <div class="bg-green-50 border border-green-200 text-green-700 px-4 py-2 rounded-lg mb-4 text-sm text-center">
        {{ session('success') }}
      </div>
    @endif

    {{-- 🧩 FORM UPDATE --}}
    <form action="{{ route('admin.kategori-klinis.update', $kategori->idkategori_klinis) }}" method="POST" class="space-y-4">
      @csrf
      @method('PUT')
